...
if(isset($_GET['id_projeto'])) {
    ...
    $sql = "SELECT * FROM equipamento_nacional WHERE projeto_id = $id_projeto ";
    ...
    if(mysqli_num_rows($resultado) > 0) {
        ...
        while($row = mysqli_fetch_assoc($resultado)) {
            ...
            $html .= '<tr>';
            $html .= '<td>' . $row['n_equipamento_nacional'] . '</td>';
            $html .= '<td>' . $row['nota_fiscal_equipamento_nacional'] . '</td>';

            $html .= '<td>' . $row['valor_equipamento_nacional'] . '</td>';
            $html .= '<td>' . $row['especificacao_equipamento_nacional'] . '</td>';
            $html .= '</tr>';
        }
